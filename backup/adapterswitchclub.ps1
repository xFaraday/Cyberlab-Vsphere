...
#importing students
$csv = Import-csv -Path 'C:\Users\administrator\Documents\Cyberlab-Vsphere-master\2022\meta3.csv'
...
$mdf=Get-Folder -Server $serv -Name "Meta3Ciso" -Location "03-Production"
...
foreach ($person in $studentprofile) {
    ...
    foreach ($vm in $vms) {
        if (($vm).Name -eq "Kali") {
        $vm | get-networkadapter | where {$_.NetworkName -ne 'DH-NAT-USERS'} | set-networkadapter -NetworkName "meta3" -confirm:$false -RunAsync:$true
